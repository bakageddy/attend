...
public class BatchDataAddRequest implements Extractor<BatchDataAddRequest> {
	public static Result<BatchDataAddRequest, Err> extract(Map<String, String[]> map) {
		...
		BatchDataAddRequest request = new BatchDataAddRequest();
		...
		if (batchid == null || (rollno == null && rollnos == null)) {
			return Result.err(new Err(
				ErrKind.IllegalArgument,
				"Cannot add to batch without either `batchid` | (`rollno` | `rollnos`)"
			));
		}
		if (batchid != null) {
			if (batchid.length != 1) {
